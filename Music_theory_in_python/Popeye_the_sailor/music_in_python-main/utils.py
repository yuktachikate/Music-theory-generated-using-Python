...
def apply_pedal(note_values, bar_value):
    ...
    '''
    print("notes :", note_values)
    print("sum of notes :", sum(note_values)) #math.fsum
    print("sum of notes2 :", math.fsum(note_values)) #math.fsum
    print("Check :", sum(note_values) % bar_value)
    assert math.fsum(note_values) % bar_value == 0
    new_values = []
    start = 0
    while True:
        cum_value = np.cumsum(np.array(note_values[start:]))
        print("Cumulative Value :",cum_value)
        print(" np.where(cum_value == bar_value) :",  np.where(cum_value == bar_value))
        # print(" np.where(cum_value == bar_value)[0][0] :",  np.where(cum_value == bar_value))[0][0]
